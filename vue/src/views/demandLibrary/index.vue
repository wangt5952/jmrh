...
<div class="tab-container">
  ...
  <el-table v-loading="loading" ref="multipleTable" @selection-change="handleSelectionChange" class="tableH" :data="list" border style="margin-top:5px;width:100%;font-size:12px;">
    ...
    <el-table-column align="center" label="需求名称">
      <template slot-scope="scope">
                    <span>
                        {{ scope.row.reqName}}</span>
                </template>
    </el-table-column>
...
export default {
  ...
  created(){
    this.pldcUrl = this.docUrl + '/lib/exportLib?objName='+this.input.objName+'&checkStatus=1&userType=7&creditLevel=&status='+this.input.status+'&token='+window.sessionStorage.getItem('token')
  },
  async mounted() {
